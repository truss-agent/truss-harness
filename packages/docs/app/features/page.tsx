...
const features = [
  ...
  ["Cloud BYOK", "Use your own API key with supported cloud providers while keeping provider selection and requests in the same provider-neutral runtime."],
...
export const metadata = createPageMetadata({ title: "Features", description: `See how ${brand.productName} helps you plan, edit, and review code with local or BYOK cloud models.`, path: "/features" });

export default function FeaturesPage() {
  return <div className="site"><SiteHeader /><main className="site-page features-page"><header className="site-page-intro"><p className="site-eyebrow">Current capabilities</p><h1>One local-first agent platform, ready in every workspace.</h1><p>Truss keeps the runtime independent from its interfaces. Its CLI, terminal UI, VS Code extension, and desktop app share the same model configuration, tools, permissions, and durable workspace state.</p></header><section id="clients" className="site-client-summary" aria-labelledby="client-summary-heading"><div><p className="site-eyebrow">Available today</p><h2 id="client-summary-heading">Four clients. One runtime.</h2></div><p><strong>CLI</strong> for automation, <strong>TUI</strong> for terminal-native work, <strong>VS Code</strong> for editor workflows, and a <strong>desktop app</strong> for a dedicated workspace.</p></section><section className="site-feature-grid">{features.map(([title, description], index) => <article key={title} className="site-feature"><span>{String(index + 1).padStart(2, "0")}</span><h2>{title}</h2><p>{description}</p></article>)}</section><section className="site-callout"><div><p className="site-eyebrow">Implementation details</p><h2>See the interfaces, configuration, and local-first setup.</h2></div><Link className="site-button site-button-primary" href="/docs">Open documentation</Link></section></main><SiteFooter /></div>;
}
