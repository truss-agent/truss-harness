...
- Chat mode exposes `read_file`, `list_directory`, `search_files`, and `grep` for grounded codebase conversations.
- Plan mode exposes the same read-only tools and saves a requested implementation checklist.
