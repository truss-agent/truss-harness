...
Select **Agents** in the main work area to create workspace-local profiles and
run them independently. Each profile can use a different local or BYOK provider,
model, mode, and approval policy. Chat and Plan work can run in parallel; Edit
runs queue behind the active workspace writer. The control center shows recent
run state, progress, approvals, errors, and verified changed files. See
[Managed Agents](/docs/runtime/managed-agents) for run policy and retention.
