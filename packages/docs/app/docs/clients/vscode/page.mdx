...
Download the signed **Truss** `.vsix` from the [Downloads page](/download), install it from the VS Code Extensions menu, then open its Activity Bar icon to start the side panel. No separate CLI installation is required. Advanced users can set `trussHarness.command` to use an external `truss-cli` executable instead of the bundled service.
...
Restart or reload VS Code if prompted. Truss checks GitHub Releases for updates
at most once per day; run **Truss: Check for Updates** to check immediately.
