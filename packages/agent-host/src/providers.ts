...
export class AgentProviderRegistry {
  ...
  async validate(
    ...
    const factory = this.factory(binding.providerId);
  ...
  private factory(id: string): AgentProviderFactory {
    const factory = this.get(id);
    if (!factory)
      throw new Error(`No agent provider is registered for '${id}'.`);
    return factory;
  }
}
...
function localProviderFactory(options: {
  ...
  return {
    descriptor: { id, label, requiresCredential: false, defaultEndpointUrl },
    async validate(binding) {
      if (!binding.modelId.trim())
        throw new Error(`${label} requires a model.`);
      try {
        const url = new URL(endpoint(binding, defaultEndpointUrl));
