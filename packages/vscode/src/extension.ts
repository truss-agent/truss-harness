...
function isConfiguration(value: unknown): value is Omit<ModelConfiguration, "mode" | "permission" | "contextWindow"> & Partial<Pick<ModelConfiguration, "mode" | "permission" | "contextWindow">> {
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const startService = (): RuntimeService => {
    ...
    const developmentCli = resolve(dirname(fileURLToPath(import.meta.url)), "../../cli/dist/bin.js");
...
<script nonce="${nonce}">
  ...
  const configurationValue = () => ({ provider: provider.value, baseUrl: endpoint.value.trim(), model: model.value.trim(), mode: configuration ? configuration.mode : 'chat', permission: permission.value, contextWindow: configuredContextWindow() });
