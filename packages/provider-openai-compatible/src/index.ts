import { ApiKeyCredential, ModelRequestError } from "@truss-harness/runtime";
import type {
  ChatMessage,
  JsonObject,
  ModelProvider,
...
function rateLimitMessage(label: string, model: string, detail: string): string {
...
function requestFailureMessage(label: string, status: number): string {
  ...
  if (status >= 500)
    return `${label} is temporarily unavailable (${status}).`;
