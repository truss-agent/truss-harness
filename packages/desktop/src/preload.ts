...
const bridge: DesktopBridge = {
  ...
  sendChat: (input: { readonly prompt: string; readonly conversationId: string; readonly history: readonly DesktopMessage[]; readonly attachments?: readonly import("@truss-harness/runtime").ChatAttachment[]; readonly activeFilePath?: string; readonly attachedPaths?: readonly string[]; readonly openFilePaths?: readonly string[] }) => ipcRenderer.invoke("truss:send-chat", input),
  ...
  createWorkspaceFile: (path: string) => ipcRenderer.invoke("truss:create-workspace-file", path),
  createWorkspaceFolder: (path: string) => ipcRenderer.invoke("truss:create-workspace-folder", path),
  renameWorkspaceEntry: (path: string, nextPath: string) => ipcRenderer.invoke("truss:rename-workspace-entry", path, nextPath),
  copyWorkspaceEntry: (path: string, destinationPath: string) => ipcRenderer.invoke("truss:copy-workspace-entry", path, destinationPath),
  deleteWorkspaceEntry: (path: string) => ipcRenderer.invoke("truss:delete-workspace-entry", path),
  revealWorkspaceEntry: (path: string) => ipcRenderer.invoke("truss:reveal-workspace-entry", path),
