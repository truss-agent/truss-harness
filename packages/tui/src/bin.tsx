...
function lines(value: string, count: number): string[] {
  return value.split(/\r?\n/).slice(0, count).map((line) => truncate(line, 88));
}
...
function Panel({ title, active, children }: { readonly title: string; readonly active: boolean; readonly children: React.ReactNode }): React.ReactElement {
  return <Box flexDirection="column" borderStyle="round" borderColor={active ? "cyan" : "gray"} paddingX={1} flexGrow={1} overflow="hidden">
...
function App({ initialConfiguration }: { readonly initialConfiguration?: ResolvedConfiguration }): React.ReactElement {
  ...
  useInput((input, key) => {
    ...
    if (screen === "command") {
      ...
      if (key.return) {
        ...
        setCommandInput("");
        setScreen("workspace");
        if (command) void runCommand(command, workspaceRoot, appendTerminal).catch((error: unknown) => appendTerminal(`[terminal error] ${String(error)}`));
        return;
      }
  ...
  return <Box flexDirection="column" height="100%" paddingX={1}>
    ...
    <Box flexGrow={1} flexDirection="row" gap={1}>
      <Box flexGrow={1}><Panel title="FILES  [Tab]" active={focus === "files"}>
        {files.slice(Math.max(0, fileIndex - 8), fileIndex + 12).map((entry) => <Text key={entry.path} color={files[fileIndex]?.path === entry.path ? "cyan" : undefined}>{files[fileIndex]?.path === entry.path ? "> " : "  "}{truncate(entry.path, 34)}</Text>)}
      </Panel></Box>
      <Box flexGrow={2}><Panel title={`${editorTitle}  [d] diff`} active={focus === "editor"}>
        {lines(editor, 28).map((line, index) => <Text key={`${index}-${line}`} color={isDiff ? (line.startsWith("+") ? "green" : line.startsWith("-") ? "red" : undefined) : undefined}>{String(index + 1).padStart(3)} {line}</Text>)}
      </Panel></Box>
      <Box flexGrow={1}><Panel title="AGENT  [Enter] send" active={focus === "chat"}>
        {activePlan && <Box flexDirection="column" marginBottom={1}><Text bold color="cyan">PLAN: {truncate(activePlan.title, 34)}</Text>{activePlan.steps.slice(0, 6).map((step) => <Text key={step.id} color={step.status === "completed" ? "green" : step.status === "in_progress" ? "yellow" : "gray"}>{step.status === "completed" ? "[x]" : step.status === "in_progress" ? "[..]" : "[ ]"} {truncate(step.content, 38)}</Text>)}</Box>}
        {chat.slice(-10).map((message, index) => <Box key={`${message.role}-${index}`} flexDirection="column" marginBottom={1}><Text color={message.role === "user" ? "cyan" : "green"} bold>{message.role === "user" ? "YOU" : "AGENT"}</Text><Text wrap="wrap">{message.content || (busy && message.role === "assistant" ? "Thinking..." : "")}</Text></Box>)}
        <Box flexGrow={1} />
        <Box><Text color={focus === "chat" ? "cyan" : "gray"}>{focus === "chat" ? "> " : "  "}{chatInput || "Ask about this workspace"}</Text></Box>
      </Panel></Box>
    </Box>
    <Box height={9} marginTop={1}><Panel title="TERMINAL  [Tab then !]" active={focus === "terminal"}>
      {terminalLines.slice(-6).map((line, index) => <Text key={`${index}-${line}`} color={line.startsWith("[agent error]") || line.startsWith("[terminal error]") ? "red" : line.startsWith("[tool") ? "yellow" : "gray"}>{truncate(line, 150)}</Text>)}
    </Panel></Box>
    <Box marginTop={1} justifyContent="space-between"><Text color="gray">Tab focus  |  m model  |  d diff  |  ! shell  |  ? help  |  Esc stop  |  Ctrl+C exit</Text><Text color="gray">{sessionId ? `session ${sessionId.slice(0, 8)}` : "new session"}</Text></Box>
