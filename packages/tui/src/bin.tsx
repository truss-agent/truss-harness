...
type ChatDisplayLine = {
  readonly role: ChatMessage["role"];
  readonly text: string;
  readonly header: boolean;
};
type EditorDisplayRow = {
...
function chatDisplayLines(
  ...
  const transcript = messages.flatMap((message) => {
    ...
    return [
      ...
      ...wrapText(content, width).map((text) => ({
        role: message.role,
        text,
        header: false,
      })),
...
function configuredContextWindow(): number {
  const configured = Number.parseInt(
    process.env.TRUSS_HARNESS_CONTEXT_WINDOW ?? "8192",
    10,
  );
...
async function collectFiles(
  ...
  for (const entry of await readdir(current, { withFileTypes: true })) {
    ...
    if (entry.isDirectory() && !ignoredDirectories.has(entry.name))
      await collectFiles(root, join(current, entry.name), result);
    if (entry.isFile())
...
function App({
  ...
  const [terminalLines, setTerminalLines] = useState<string[]>([
    "Terminal ready. Shell commands run in the workspace root; slash commands run locally.",
  ]);
  ...
  const fileSearchResults = useMemo(
    () =>
      fuzzyFiles(
        files,
        fileSearchInput,
        Math.max(6, Math.min(14, viewport.rows - 10)),
      ),
  ...
  useEffect(() => {
    void collectFiles(workspaceRoot)
      .then(setFiles)
      .catch((error: unknown) =>
  ...
  useEffect(() => {
    ...
    void listLocalModels(endpoint)
      .then((available) => {
        ...
        const selected = available.findIndex(
          (model) => model.name === modelInput,
        );
  ...
  useInput((input, key) => {
    ...
    if (screen === "file-search") {
      ...
      if (key.downArrow) {
        setFileSearchIndex((current) =>
          Math.min(Math.max(0, fileSearchResults.length - 1), current + 1),
        );
        return;
      }
      if (key.return) {
        const result =
          fileSearchResults[
            Math.min(fileSearchIndex, Math.max(0, fileSearchResults.length - 1))
          ];
    ...
    if (screen === "agents") {
      if (key.escape || input.toLowerCase() === "a") setScreen("workspace");
    ...
    if (screen === "settings") {
      ...
      if (settingsField === "model") {
        ...
        if (key.downArrow)
          setModelIndex((current) => Math.min(models.length - 1, current + 1));
        if (models[modelIndex] && (key.return || key.rightArrow))
          setModelInput(models[modelIndex]);
      }
      if (settingsField === "internet") {
        if (input === " " || key.leftArrow || key.rightArrow)
          setInternetAccess((current) => !current);
      ...
      if (key.return && endpointInput && modelInput) {
        void configureRuntime();
        return;
      }
      const setter =
        settingsField === "endpoint" ? setEndpointInput : setModelInput;
      if (key.backspace || key.delete)
    ...
    if (focus === "terminal") {
      if (key.upArrow) {
        setTerminalScroll((current) =>
          clamp(
            current + 1,
            0,
            Math.max(0, terminalLines.length - Math.max(1, terminalHeight - 4)),
          ),
        );
        return;
      }
      ...
      if (key.return) {
        ...
        setCommandInput("");
        if (command)
          void runTerminalInput(command).catch((error: unknown) =>
    ...
    if (focus === "files") {
      ...
      if (key.downArrow)
        setFileIndex((current) =>
          Math.min(Math.max(fileTree.length - 1, 0), current + 1),
        );
      if (key.leftArrow && selectedFileTreeEntry?.kind === "directory")
        toggleDirectory(selectedFileTreeEntry.path, false);
      if (key.rightArrow && selectedFileTreeEntry?.kind === "directory")
        toggleDirectory(selectedFileTreeEntry.path, true);
      if (key.return && selectedFileTreeEntry?.kind === "directory")
        toggleDirectory(selectedFileTreeEntry.path);
      if (key.return && selectedFileTreeEntry?.kind === "file")
        void loadFile(selectedFileTreeEntry);
      return;
    }
    if (focus === "editor") {
      if (key.upArrow) {
        setEditorScroll((current) =>
          clamp(
            current - 1,
            0,
            Math.max(0, editorRows.length - editorLineCount),
          ),
        );
        return;
      }
    ...
    if (key.upArrow) {
      setChatScroll((current) =>
        clamp(
          current + 1,
          0,
          Math.max(0, chatTranscript.length - chatLineCount),
        ),
      );
      return;
    }
  ...
  const editorPanel = (
    ...
    >
      {visibleEditorRows.map((row, index) => {
        ...
        return (
          <Text
            key={`${editorScroll + index}-${row.sourceLine}-${row.continuation}`}
            wrap="truncate-end"
            color={theme.text}
          >
  ...
  const agentPanel = (
    ...
    >
      ...
      {chatLines.map((line, index) => (
        <Text
          key={`${line.role}-${line.header}-${index}-${line.text}`}
      ...
      <Box height={terminalHeight} marginTop={1}>
        ...
        >
          {visibleTerminalLines.map((line, index) => (
            ...
            >
              {truncate(line, Math.max(20, viewport.columns - 8))}
            </Text>
          ))}
          <Box flexGrow={1} />
          <Text
